...
/**
 * This is a subclass of <tt>NumericDataSet</tt> (which, in turn, is a subclass
 * of <tt>DataSet</tt>) representing a dataset all of whose attributes are
 * binary, i.e., numeric with only the two values 0 and 1. In addition to all of
 * the fields and methods inherited from <tt>DataSet</tt>, the class includes a
 * constructor that reads in data from data files and converts all attributes to
 * binary.
 **/
public class BinaryDataSet extends NumericDataSet {
	...
	public BinaryDataSet(String filestem) throws FileNotFoundException, IOException {
			...
			for (int i = 0; i < exs.length; i++) {
				...
				for (int a = 0, na = 0; a < numAttrs; a++) {
					for (int j = 1; j < cont_vals[a].length; j++)
						new_ex[na++] = (exs[i][a] >= cont_vals[a][j] ? 1 : 0);
				}
				exs[i] = new_ex;
			}
		}
		...
		for (int a = 0, na = 0; a < numAttrs; a++) {
			if (cont_vals[a].length == 2 && cont_vals[a][0] == 0 && cont_vals[a][1] == 1)
				new_attrName[na++] = attrName[a];
			else
				for (int j = 1; j < cont_vals[a].length; j++)
					new_attrName[na++] = attrName[a] + ">=" + cont_vals[a][j];
		}
